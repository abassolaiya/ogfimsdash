...
const CommodityPriceList = () => {
  ...
  useEffect(() => {
    const fetchCommodities = async () => {
      try {
        const response = await fetch(`${baseURL}commodity-prices/latest`);
        const data = await response.json();
        console.log(data);

        // Ensure commodities is an array before setting state
        if (data.success && Array.isArray(data.commodities)) {
          setCommodities(data.commodities);
        } else {
          setCommodities([]); // Set to empty array if no valid data
        }
      } catch (error) {
    ...
    <div>
      ...
      <div className="commodity-price-list">
        {commodities.map((commodity) => {
          const recentPrice =
            commodity.recentPrices && commodity.recentPrices[0]?.price;
              ...
              <div className="commodity-box">
                ...
                <div className="bottom-section">
                  <div className="local-gov-prices">
                    {commodity.recentPrices
                      .slice(0, 3)
                      .map((priceInfo, index) => (
                        <div className="lga-box" key={index}>
                          <span className="lga-name">{priceInfo.lga}</span>
                          <span className="lga-price">₦{priceInfo.price}</span>
                          <span className="last-updated">
                            {new Date(priceInfo.updatedAt).toLocaleDateString()}
                          </span>
                        </div>
                      ))}
                  </div>
